    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md mx-4 rounded-xl border border-forest-border bg-forest-card p-6 shadow-xl">
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button type="button" onClick={onCancel} className="px-4 py-2 rounded-xl text-sm border border-forest-border text-forest-light-gray hover:text-white transition-colors">
            Cancel
          </button>
          <button type="button" onClick={() => onConfirm(reason.trim())} className="px-4 py-2 rounded-xl text-sm bg-forest-emerald text-forest-darker font-medium hover:brightness-110 transition-all">
            Skip concept
          </button>
        </div>
      </div>
