 ...
 * DOM rects relative to the container (for absolute positioning inside it).
 */
...
const StudyPanel = React.memo(({
  ...
  onToggleTurnInclusion,
  onToggleMemoryEffective,
  ...
  const includeParentContext = activeNode?.includeParentContext !== false
  const memoryPath = useMemo(
    () => (includeParentContext ? breadcrumbPath : breadcrumbPath.slice(-1)),
    [includeParentContext, breadcrumbPath]
  )
  const effectiveMemories = useMemo(() => collectEffectiveMemories(memoryPath), [memoryPath])

  const hasAssistantContent = messages.some((m) => m.role === 'assistant')
  const isDefaultNodeLabel = /^Node\s*\d+$/i.test((activeNode?.label || '').trim())
  ...
  useEffect(() => {
    const scrollEl = chatScrollRef.current
    if (!scrollEl) return
    const onScroll = () => computeOverlays()
  ...
  useEffect(() => {
    ...
    if (!el) return
    const ro = new ResizeObserver(() => setPanelRect(el.getBoundingClientRect()))
    ro.observe(el)
  ...
  const handleSubmitQuestion = (e) => {
    ...
    if (!isAILoading && activeNode && (hasText || hasImage)) {
      ...
      onAskQuestion(
        activeNode.id,
        text,
        attachedImage ? { mimeType: attachedImage.mimeType, data: attachedImage.data } : null,
      )
      setQuestion('')
  ...
  const saveLabel = () => {
    ...
    if (trimmed && activeNode && onRenameNode && !schemaBlocked) {
  ...
  const handleAddMemorySubmit = (e) => {
    e.preventDefault()
    if (schemaBlocked || !activeNode) return

    const title = memoryTitle.trim()
    const reason = memoryReason.trim()
    const content = memoryContent.trim()

    if (!title || !content) return

    onAddMemory?.(activeNode.id, { title, reason, content })
    setMemoryTitle('')
    setMemoryReason('')
    setMemoryContent('')
  }

  const handleEditMemory = (entry) => {
    if (schemaBlocked || !activeNode || entry.sourceNodeId !== activeNode.id) return

    const nextTitle = window.prompt('What are you storing?', entry.memory.title)
    if (nextTitle === null) return

    const nextReason = window.prompt('Why is it important?', entry.memory.reason || '')
    if (nextReason === null) return

    const nextContent = window.prompt('Content to always share', entry.memory.content || '')
    if (nextContent === null) return

    onUpdateMemory?.(activeNode.id, entry.memory.id, {
      title: nextTitle.trim() || entry.memory.title,
      reason: nextReason.trim(),
      content: nextContent.trim() || entry.memory.content,
    })
  }
  ...
  if (!activeNode && hasNoNodesYet) {
    const handleFirstSubmit = (e) => {
      ...
      if (!schemaBlocked && text && onCreateFirstNode && !isAILoading) {
    ...
    return (
      <div className="h-full flex flex-col bg-forest-darker">
        {schemaErrorMessage && (
          <div className="mx-6 mt-4 rounded-lg border border-amber-500/60 bg-amber-900/50 px-4 py-2 text-sm text-amber-100">
            {schemaErrorMessage}
          </div>
        )}
        ...
        <div className="flex-shrink-0 border-t border-forest-border bg-forest-card/50 p-4">
          <form onSubmit={handleFirstSubmit} className="flex gap-3 max-w-2xl mx-auto">
            <input
              ...
              disabled={isAILoading || schemaBlocked}
              className="flex-1 px-4 py-3 bg-forest-darker border border-forest-border rounded-xl text-white placeholder-forest-gray focus:outline-none focus:border-forest-emerald transition-colors disabled:opacity-60"
            />
            <button
              type="submit"
              disabled={isAILoading || schemaBlocked || !firstChatInput.trim()}
  ...
  if (!activeNode) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-8 bg-forest-darker">
        <motion.div
        ...
        >
          ...
          <p className="text-forest-light-gray mb-4">Select a node from canvas mode, then open chat.</p>
        </motion.div>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex bg-forest-darker">
      {/* Chat column */}
      <div ref={panelRef} className="flex-1 min-w-0 flex flex-col relative">
                      ...
                      isEditingLabel ? (
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <input
                            ...
                            className="px-2 py-1 text-sm font-semibold text-forest-emerald bg-forest-darker border border-forest-emerald/50 rounded-md min-w-[120px] focus:outline-none focus:ring-1 focus:ring-forest-emerald disabled:opacity-60"
                          />
                        </div>
                      ) : (
                        <div className="flex items-center gap-1.5 flex-shrink-0 group">
                          <span className="text-forest-emerald font-semibold whitespace-nowrap">{node.label}</span>
                          <button
                            type="button"
                            onClick={startEditingLabel}
                            disabled={schemaBlocked}
                            className="p-1 rounded text-forest-gray hover:text-forest-emerald hover:bg-forest-border/50 opacity-70 group-hover:opacity-100 transition-opacity disabled:opacity-30"
                            title="Rename node"
                          >
                            <Pencil size={12} />
                          </button>
        ...
        <div className="flex-1 overflow-hidden flex flex-col relative">
          <div ref={chatScrollRef} className="flex-1 overflow-y-auto overflow-x-hidden min-h-0">
            <div
              ref={contentRef}
            ...
            >
              {messages.length === 0 && !isAILoading ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-forest-gray">Ask a question below to start learning about this topic.</p>
                </div>
              ) : (
                <>
                  {messages.map((msg, i) => (
                    <MemoizedMessage
                      key={`${activeNode.id}-${msg.id || i}`}
                      msg={msg}
              ...
              {highlightOverlays.length > 0 && (
                <div className="highlight-inline-layer">
                  {highlightOverlays.map(({ childId, label, rects }) =>
                    rects.map((r, i) => (
                      <div
                        ...
                        title={`Jump to: ${label}`}
                        className="highlight-inline-box"
                        style={{
                          top: r.top,
                          left: r.left,
                          width: r.width,
              ...
              <div className="px-4 py-3 border-t border-forest-border/50 bg-forest-card/30 flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={schemaBlocked}
                    className="flex-1 px-3 py-2 bg-forest-emerald/20 border border-forest-emerald/50 text-forest-emerald rounded-lg text-sm font-medium hover:bg-forest-emerald/30 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    <GitBranch size={14} />
                    Create new node
                  </button>
                  <button
                    ...
                    disabled={schemaBlocked}
                    className="px-3 py-2 text-forest-gray hover:text-white text-sm disabled:opacity-50"
                  >
                    Dismiss
                  </button>
                </div>
                ...
                {!showBranchInput ? (
                  <div className="p-2">
                    ...
                    <button
                      ...
                      disabled={schemaBlocked}
                      className="w-full px-4 py-2.5 bg-forest-emerald text-forest-darker rounded-lg hover:brightness-110 transition-all flex items-center justify-center gap-2 text-sm font-semibold disabled:opacity-50"
                    >
                      <TreePine size={16} />
                      Ask Forest
                    </button>
                  ...
                  <form onSubmit={handleBranchSubmit} className="p-3 space-y-3">
                    ...
                    <input
                      ...
                      disabled={isAILoading || schemaBlocked}
                      autoFocus
                      className="w-full px-3 py-2.5 bg-forest-darker border border-forest-border rounded-lg text-white placeholder-forest-gray text-sm focus:outline-none focus:border-forest-emerald disabled:opacity-60"
                    />
                    <div className="flex gap-2">
                      <button
                        type="button"
                      ...
                      <button
                        type="submit"
                        disabled={isAILoading || schemaBlocked || !branchQuestion.trim()}
        ...
        <div className="flex-shrink-0 border-t border-forest-border bg-forest-card/50 p-4">
          <form onSubmit={handleSubmitQuestion} className="flex gap-3">
            <div className="flex-1 flex flex-col gap-1.5">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={schemaBlocked}
                  className="flex-shrink-0 p-3 rounded-xl border border-forest-border bg-forest-darker text-forest-gray hover:text-forest-emerald hover:border-forest-emerald/50 transition-colors disabled:opacity-50"
            ...
            <button
              type="submit"
              disabled={schemaBlocked || isAILoading || (!question.trim() && !attachedImage)}
              className="px-5 py-3 bg-forest-emerald text-forest-darker rounded-xl hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 font-medium"
            >
              {isAILoading ? (
                <>
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            {breadcrumbPath.length > 1 && (
              <p className="text-xs text-forest-gray">
                {includeParentContext ? 'Parent context enabled' : 'Parent context disabled'}
              </p>
            )}
          </div>
        </div>
      ...
      {/* Right context rail */}
      <aside className="w-[360px] max-w-[38vw] min-w-[300px] border-l border-forest-border bg-forest-card/40 backdrop-blur-sm flex flex-col">
        <div className="px-4 py-3 border-b border-forest-border/70">
          <h3 className="text-sm font-semibold text-white flex items-center gap-2">
            <Database size={16} className="text-forest-emerald" />
            Context Control
          </h3>
          <p className="text-xs text-forest-gray mt-1">Toggle what gets sent to the model.</p>
          {schemaErrorMessage && (
            <p className="text-xs text-amber-300 mt-2">{schemaErrorMessage}</p>
          )}
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          <section>
            <h4 className="text-xs uppercase tracking-wider text-forest-gray mb-2">Parent Inheritance</h4>
            <button
              type="button"
              onClick={() => onToggleParentContext?.(activeNode.id)}
              disabled={schemaBlocked}
              className={`w-full text-left p-3 rounded-lg border transition-colors disabled:opacity-50 ${
                includeParentContext
                  ? 'border-forest-emerald/60 bg-forest-emerald/10'
                  : 'border-forest-border bg-forest-darker/60'
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm text-white font-medium">
                  {includeParentContext ? 'Include parent context' : 'Parent context disabled'}
                </p>
                {includeParentContext ? (
                  <CheckCircle2 size={14} className="text-forest-emerald" />
                ) : (
                  <Circle size={14} className="text-forest-gray" />
                )}
              </div>
              <p className="text-xs text-forest-gray mt-1">
                When off, inherited parent path and parent memories are excluded from model context.
              </p>
            </button>
          </section>

          <section>
            <h4 className="text-xs uppercase tracking-wider text-forest-gray mb-2">Context Turns</h4>
            <div className="space-y-2">
              {turns.length === 0 ? (
                <p className="text-xs text-forest-gray">No turns yet.</p>
              ) : (
                turns.map((turn, index) => {
                  const summary = summarizeTurn(turn)
                  const isOn = turn.includeInContext !== false
                  return (
                    <button
                      key={turn.id}
                      type="button"
                      onClick={() => onToggleTurnInclusion?.(activeNode.id, turn.id)}
                      disabled={schemaBlocked}
                      className={`w-full text-left p-3 rounded-lg border transition-colors disabled:opacity-50 ${
                        isOn
                          ? 'border-forest-emerald/60 bg-forest-emerald/10'
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        <p className="text-xs text-forest-light-gray font-medium">Turn {index + 1}</p>
                        {isOn ? <CheckCircle2 size={14} className="text-forest-emerald" /> : <Circle size={14} className="text-forest-gray" />}
                      </div>
                      <p className="text-xs text-forest-light-gray/90"><span className="text-forest-gray">Q:</span> {summary.question}</p>
                      <p className="text-xs text-forest-light-gray/80 mt-1"><span className="text-forest-gray">A:</span> {summary.answer}</p>
                    </button>
                  )
                })
              )}
          ...
          <section>
            <h4 className="text-xs uppercase tracking-wider text-forest-gray mb-2">Persistent Branch Memory</h4>
            <div className="space-y-2">
              {effectiveMemories.length === 0 ? (
                <p className="text-xs text-forest-gray">No memory items yet.</p>
              ) : (
                effectiveMemories.map((entry) => {
                  const isOn = entry.effectiveEnabled !== false
                  const isSourceNode = entry.sourceNodeId === activeNode.id
                  const overrideKey = memoryOverrideKey(entry.sourceNodeId, entry.memoryId)
                  const overriddenHere = !isSourceNode && Object.prototype.hasOwnProperty.call(activeNode.memoryOverrides || {}, overrideKey)
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-lg border p-3 ${
                        isOn
                          ? 'border-forest-emerald/50 bg-forest-emerald/10'
                          : 'border-forest-border bg-forest-darker/60'
                      }`}
                    >
                      <button
                        type="button"
                        onClick={() => onToggleMemoryEffective?.(activeNode.id, entry.memoryId, entry.sourceNodeId)}
                        disabled={schemaBlocked}
                        className="w-full text-left"
                      >
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm text-white font-medium truncate">{entry.memory.title || 'Untitled memory'}</p>
                          {isOn ? <CheckCircle2 size={14} className="text-forest-emerald flex-shrink-0" /> : <Circle size={14} className="text-forest-gray flex-shrink-0" />}
                        </div>
                        <p className="text-[11px] text-forest-gray mt-1">
                          {isSourceNode ? 'Current node memory' : `Inherited from: ${entry.sourceNodeLabel}`}
                          {overriddenHere ? ' · Overridden here' : ''}
                        </p>
                        {entry.memory.reason && (
                          <p className="text-xs text-forest-light-gray mt-2"><span className="text-forest-gray">Why:</span> {entry.memory.reason}</p>
                        )}
                        {entry.memory.content && (
                          <p className="text-xs text-forest-light-gray mt-1 line-clamp-4">{entry.memory.content}</p>
                        )}
                      </button>
                      ...
                      {isSourceNode && (
                        <div className="flex items-center gap-2 mt-2">
                          <button
                            ...
                            disabled={schemaBlocked}
                            className="px-2 py-1 rounded-md text-xs border border-forest-border text-forest-light-gray hover:text-white hover:border-forest-emerald/60 flex items-center gap-1 disabled:opacity-50"
                          >
                            <Edit3 size={12} />
                            Edit
                          </button>
                          <button
                            type="button"
                            onClick={() => onDeleteMemory?.(activeNode.id, entry.memory.id)}
                            disabled={schemaBlocked}
                            className="px-2 py-1 rounded-md text-xs border border-red-600/50 text-red-300 hover:bg-red-900/40 flex items-center gap-1 disabled:opacity-50"
                          >
                            <Trash2 size={12} />
                            Delete
                          </button>
          ...
          <section className="pt-2 border-t border-forest-border/50">
            <h4 className="text-xs uppercase tracking-wider text-forest-gray mb-2">Add Memory</h4>
            <form onSubmit={handleAddMemorySubmit} className="space-y-2.5">
              <input
                type="text"
                value={memoryTitle}
                ...
                disabled={schemaBlocked}
                className="w-full px-3 py-2 text-sm bg-forest-darker border border-forest-border rounded-lg text-white placeholder:text-forest-gray focus:outline-none focus:ring-1 focus:ring-forest-emerald"
              />
              <input
                type="text"
                value={memoryReason}
              ...
              <textarea
                ...
                placeholder="Content to always share in this branch and child nodes"
                rows={4}
                disabled={schemaBlocked}
                className="w-full px-3 py-2 text-sm bg-forest-darker border border-forest-border rounded-lg text-white placeholder:text-forest-gray focus:outline-none focus:ring-1 focus:ring-forest-emerald resize-y"
              />
              <button
                type="submit"
                disabled={schemaBlocked || !memoryTitle.trim() || !memoryContent.trim()}
                className="w-full px-3 py-2 rounded-lg bg-forest-emerald text-forest-darker hover:brightness-110 transition-all text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Save Memory
              </button>
            </form>
