...
export const probe = async ({ node, mode = 'near', goals = [], goalsCovered = [] }) => {
  ...
  const systemPrompt = [
    ...
    '',
    'CRITICAL — VARY THE QUESTION SHAPE EACH TURN. Look at recent tutor probes in the user prompt.',
    'If a previous probe asked "what happens to X when Y is broken/removed", DO NOT ask the same',
    'shape with a different leaf/plant. The student already showed they can answer it. Switch the',
    'angle: ask about a different goal, reverse direction (effect → required cause), compare two',
    'cases, or pick a different mechanism dimension. Treat repetition as a failure mode.',
    '',
